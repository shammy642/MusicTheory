...
export const ContentsPage = () => {
  ...
  const displayQuizesBySection = (sectionName: string) => {
    const filteredQuizes = quizes.filter(
      (quizes) => quizes.section === sectionName
    );
    return (
      <React.Fragment>
        {filteredQuizes.map((quizes, index) => (
          <Grid xs key={quizes.id}>
            {userProgressState.completedQuizes.includes(quizes.id) ? (
              <Box
                onClick={() => navigate("/quiz/" + quizes.id)}
                margin={5}
                sx={{ cursor: "pointer" }}>
                <Star color="gold">{index + 1}</Star>
              </Box>
            ) : (
              <Box
                onClick={() => navigate("/quiz/" + quizes.id)}
                margin={4}
                sx={{ cursor: "pointer" }}>
                <Star color="#8b8b8b9a">{index + 1}</Star>
              </Box>
            )}
          </Grid>
        ))}
        ...
        <Box sx={{ flexGrow: 1 }}>
          <Container>
            ...
            <Box display="flex" justifyContent="center" margin={2}>
              <Typography variant="h3" color={"#ff0095"}>
                Letter Names
              </Typography>
            </Box>
            <Box display="flex" justifyContent="center" margin={2}>
